...
const Notdienstanzeige = (props) => {
    ...
    const ladeNotdienste = () => {
        $.ajax({
            url: `/lakbwportal/xmlschnittstelle/kennziffer/ergebnis?digits=127&dayCount=1&key=1639569272519_-2008290756`,
            method: "GET",
            crossDomain: true,
            cache: false,
            headers: {
            ...
            success: function (result, code, request) {
                ...
                setState({notdienste: state.notdienste, lastRefresh: new Date(), status: "", display: state.display});
    ...
    const checkCurrentVersion = (successCallback) => {
        $.ajax({url: `https://apothekennotdienst-bw.de/lakbwportal/resources/common/js/emergencyInterface/html/version.txt`, type: 'GET', success: successCallback});
    }
